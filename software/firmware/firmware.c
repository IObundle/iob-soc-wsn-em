...
int main() { 				
  ...
  uint32_t pdu_size=0, sn_start_time=0, bs_start_time=0, elapsedu=0, j=0, sn_ch_freq=0, bs_ch_freq=0;
  uint64_t elapsed=0;   	 
  ...
  if(get_id()==sn.id) {
	...
	case MODE_SN_RX_CONNECT_REQ:
	      ...
	      // Print data for debugging purpose
              printf_("\npdu_size = %d bytes\nsize(sn_rx_connect_request_pdu) = %d bytes\n", pdu_size, sizeof(sn_rx_connect_request_pdu));	
     ...
     switch(bs.nextState) {
        ...
        case MODE_BS_TX_CONNECT_REQ:	      	  		       
	     ...
	     //txrx_set_ch_idx(bs_adv_ch_idx);	
	      ...
	      bs_tx_connect_request_pdu.connect_req_payload.LLData_Timeout=5; bs_tx_connect_request_pdu.connect_req_payload.LLData_ChM=6;		
  	...
  	case MODE_BS_RX_ADV_IND:  	      
	      ...
	      ble_off();
  	      ...
  	      // Print data for debugging purpose	      	      
	      printf_("\npdu_size = %d bytes\nsize(bs_rx_adv_ind_pdu) = %d bytes\n", pdu_size, sizeof(bs_rx_adv_ind_pdu));	
